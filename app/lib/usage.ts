...
/**
 * Result returned by the atomic audit/project reservation.
 *
 * This is the authoritative operation used by the audit API.
 */
...
/**
 * Get the currently active subscription and its plan.
 *
 * Active means:
 *
 * - status = active
 ...
 * - endsAt > now
 *
 * Expired subscriptions are marked as expired before lookup.
 */
async function getActiveSubscriptionWithPlan(
    userId: string
) {
    const now = new Date();
...
export async function getUsageSummary(
    ...
    const active =
        await getActiveSubscriptionWithPlan(
            userId
        );

    const projectsUsed =
        await getActiveProjectCount(
            userId
        );
    ...
    return {
        ...
        plan: {
            ...
            currency:
                active.plan.currency,

            interval:
                active.plan.interval,

            auditLimit:
                active.plan.auditLimit,

            pagesPerAudit:
                active.plan.pagesPerAudit,

            aiRecommendationLimit:
                active.plan.aiRecommendationLimit,

            maxProjects:
                active.plan.maxProjects,
        },
...
export async function checkAuditLimit(
    ...
    if (!active) {
        return {
            ...
            message:
                "You need an active subscription to create an audit.",
        };
    }
    ...
    if (!usage) {
        return {
            ...
            message:
                "Unable to initialize your usage period.",
        };
    }
 ...
 * ============================================================
 *
 * Kept for other server-side consumers.
 *
 * For the audit API itself, use:
 *
 *     reserveAuditAndProject()
 *
 * because audit + project creation must be atomic.
 */
export async function consumeAudit(
    ...
    if (!usage) {
        throw new Error(
            "Unable to initialize subscription usage."
        );
    }
    ...
    const updated =
        await db
            ...
            .set({
                auditsUsed:
                    sql`${subscriptionUsage.auditsUsed} + 1`,
 ...
 * ============================================================
 *
 * THIS IS THE AUTHORITATIVE OPERATION FOR THE AUDIT API.
 *
 * It atomically handles:
 *
 *   1. subscription validation
 *   2. usage-period creation
 *   3. audit quota reservation
 *   4. project lookup
 *   5. project-limit enforcement
 *   6. project creation
 *
 * PostgreSQL transaction-level advisory locking is used to
 * serialize quota/project operations for the same user.
 *
 * Therefore:
 *
 * Request A ─┐
 * Request B ─┼──> same user lock ──> serialized
 * Request C ─┘
 *
 * If the audit quota or project limit fails, the transaction
 * makes no changes.
 *
 * If project creation succeeds but audit reservation fails,
 * the transaction rolls back the project.
 *
 * If audit reservation succeeds but project creation fails,
 * the transaction rolls back the audit reservation.
 */
/**
 * ============================================================
 * ATOMIC AUDIT + PROJECT RESERVATION
...
export async function reserveAuditAndProject(
    ...
    let failureReason: ReservationFailure | null = null;

    try {
        return await db.transaction(async (tx) => {
            // 1. Serialize quota/project operations for this user using advisory lock
            await tx.execute(
            ...
            if (!active) {
                failureReason = {
                    ...
                    projectUsed: 0,
                    projectLimit: 0,
                    message: "An active subscription is required to run an SEO audit.",
                };
            ...
            const auditUsed = usage?.auditsUsed ?? 0;
            ...
            // 5. Lookup or Check Existing Project
            ...
            const finalProjectsUsed = projectCreated ? projectUsed + 1 : projectUsed;
        ...
        if (error?.message === "__AUDIT_RESERVATION_ROLLBACK__" && failureReason) {
            return {
                allowed: false,
                ...
                plan: failureReason.plan
                    ? {
                          ...
                          aiRecommendationLimit: failureReason.plan.aiRecommendationLimit,
                ...
                auditUsage: {
                    ...
                    remaining: Math.max(0, failureReason.auditLimit - failureReason.auditUsed),
                    message: failureReason.message,
                },
                projectUsage: {
                    ...
                    remaining: Math.max(0, failureReason.projectLimit - failureReason.projectUsed),
                },
            };
        }
 ...
 * ============================================================
 *
 * Called when an audit fails AFTER the reservation succeeded.
 *
 * Decrements auditsUsed by 1 (never below 0) and, if the
 * project was newly created for this failed audit, marks it
 * inactive so it stops counting against the project limit.
 *
 * Serialized against reserveAuditAndProject via the same
 * advisory lock.
 */
export async function refundAuditReservation(
  ...
  await db.transaction(async (tx) => {
    ...
    // Find the user's active subscription (already loaded by
    // reservation; we re-fetch to be safe).
    const active = (
      await tx
        .select({
          subscription: subscriptions,
    ...
    // If the project was created solely for this failed audit,
    // deactivate it so it stops counting.
...
export async function checkPageLimit(
    ...
    if (
        ...
        return {
            ...
            message:
                "Requested page count must be greater than zero.",
        };
    }
    ...
    if (!active) {
        return {
            ...
            message:
                "You need an active subscription to crawl pages.",
        };
    }
 ...
 * Reporting only.
 *
 * This does NOT enforce pagesPerAudit.
 */
export async function recordPagesCrawled(
    ...
    if (!active) {
        return {
            ...
            message:
                "You need an active subscription to record crawled pages.",
        };
    }
    ...
    const updated =
        await db
            ...
            .set({
                pagesCrawled:
                    sql`${subscriptionUsage.pagesCrawled} + ${pageCount}`,
    ...
    return {
         ...
         * Informational only.
         *
         * This is NOT period remaining usage.
         */
        limit:
            active.plan.pagesPerAudit,
...
export async function consumeAIRecommendation(
    ...
    const updated =
        await db
            ...
            .set({
                aiRecommendationsUsed:
                    sql`${subscriptionUsage.aiRecommendationsUsed} + 1`,
...
export async function checkUsageLimit(
    ...
    switch (type) {
        case "audits":
            return checkAuditLimit(
                userId
            );

        case "ai_recommendations":
            return checkAIRecommendationLimit(
                userId
            );

        case "projects":
            return checkProjectLimit(
                userId
            );

        case "pages":
            return checkPageLimit(
                userId,
                1
            );
